  ...
  _translations:
    ...
    ru:
      ...
      panels:
        title: "Подписаться"
      podcast_panel:
        choose_client: "Продолжить"
      help_panel:
        title: "Подписаться?"
        paragraph1: "Вы собираетесь подписаться на подкаст. Это позволит вашему приложению для подкастов автоматически загружать новые эпизоды или получать доступ к архиву ранее выпущенных эпизодов."
        paragraph2: "Кнопка Подписаться от Podlove поможет вам сделать это. Выберите свое любимое приложение для подкастов из списка потенциальных приложений на вашем устройстве или выберите облачную службу подкаста в интернете, который вы используете."
        paragraph3: "После запуска клиент подкаста должен предложить вам добавить подкаст в свой список подписок. Используйте ссылку для загрузки, чтобы получить приложение, если оно еще не доступно."
      clients_panel:
        app: "Приложение"
        cloud: "Облако"
        other_client: "Другое приложение"
      finish_panel:
        handing_over_to: "Отправка в %{client}"
        something_went_wrong: "Что-то пошло не так?"
        try_again: "Попробовать снова"
        install: "Установить %{client}"
        register_an_account: "Зарегистрироваться в "
        please_copy_url: "Скопируйте URL ниже и добавьте его в подкаст или RSS приложение."
        copy_button_text: "Копировать URL"
        copy_success: "URL скопирован в буфер обмена"
        or_install: "или установить приложение"
